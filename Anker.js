...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const FarhanGans = ["08993801551@s.whatsapp.net"] // ubah aja gapapa
...
//akhir kreator
		    ...
		    case 'darkjokes':
				...
				reply(mess.wait)
                 ...
                 client.sendMessage(from, darkjokes, image, {quoted: mek, caption: '\`\`\`JAY JOKES\`\`\`'})
				await limitAdd(sender) 
				break  
			case 'katailham':
				 if (isBanned) return reply(mess.only.benned) 
		    ...
		    case 'map':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔═══✪ Tag By *${pushname2}* ✪══`+ teks +'╚═══〘 JAY BOT 〙═══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 JAY BOT 〙', members_id, true)
					break
			    case 'kbbi':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
				...
				case 'clearall':
					...
					reply(`\`\`\`Sukses delete all chat JAYBOT\`\`\``)
					break
                                case 'bcgc':
					client.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
